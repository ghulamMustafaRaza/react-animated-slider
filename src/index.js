...
class Slider extends React.PureComponent {
  ...
  UNSAFE_componentWillReceiveProps(props) {
    this.updateChilds(props, this.state);
  }
  UNSAFE_componentWillMount() {
    this.updateChilds(this.props, this.state);
  }
  updateChilds = (props, state) => {
    const slideCount = React.Children.count(props.children);
    const newState = {};
    if (state.currentSlideIndex >= slideCount) {
      newState.currentSlideIndex = 0;
    }
    if (state.cp != props.children) {
      const renderHeadless = typeof props.children == "function";
      newState.children = renderHeadless
        ? props.children({
            onMouseOver: this.handleMouseOver,
            onMouseOut: this.handleMouseOut,
          })
        : props.children;
      newState.cp = props.children;
    }
  ...
  handleTouchStart = (e) => {
    ...
    const touchDelta = this.currentElement.getBoundingClientRect()[
      this.swipeProperty
    ];
    ...
    if (this.previousElement) {
      ...
      this.previousElementStartPosition =
        this.previousElement.getBoundingClientRect()[this.swipeProperty] -
        touchDelta;
    }
    if (this.nextElement) {
      ...
      // eslint-disable-next-line max-len
      this.nextElementStartPosition =
        this.nextElement.getBoundingClientRect()[this.swipeProperty] -
        touchDelta;
    }
  };
    ...
    this.animating =
      ...
      requestAnimationFrame(() => {
        ...
        this.currentElementPosition =
          this.currentElementStartPosition + newLeft;
        this.currentElement.style[
          this.swipeProperty
        ] = `${this.currentElementPosition}px`;
        if (this.previousElement) {
          this.previousElementPosition =
            this.previousElementStartPosition + newLeft;
          this.previousElement.style[
            this.swipeProperty
          ] = `${this.previousElementPosition}px`;
        }
        if (this.nextElement) {
          this.nextElementPosition = this.nextElementStartPosition + newLeft;
          this.nextElement.style[
            this.swipeProperty
          ] = `${this.nextElementPosition}px`;
        }
        this.animating = false;
      });
  };
  ...
  handleTouchEnd = () => {
    ...
    const touchDelta =
      this.currentElementStartPosition - this.currentElementPosition;
  ...
  render() {
    this.slideCount = React.Children.count(this.state.children);
    const {
      children,
      className = "slider",
      previousButton = (
        <Arrow direction={this.direction === HORIZONTAL ? "left" : "down"} />
      ),
      nextButton = (
        <Arrow direction={this.direction === HORIZONTAL ? "right" : "up"} />
      ),
      touchDisabled,
      autoplay,
    } = this.props;
    ...
    const mouseEvents = autoplay && {
      onMouseOver: this.handleMouseOver,
      onMouseOut: this.handleMouseOut,
    };
    const renderHeadless = typeof children == "function";

    return (
      <div
        className={classNames.slider || className}
        {...(!touchDisabled && { ref: this.initTouchEvents })}
        {...(!renderHeadless && mouseEvents)}
      >
        <a
          {...(renderHeadless && mouseEvents)}
        ...
        <div className={classNames.track}>
          {React.Children.map(this.state.children, (item, index) =>
